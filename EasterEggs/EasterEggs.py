...
# El programa le muestra al usuario los datos que ha almacenado en el sistema. Igualmente, le permite exportar un archivo .csv con esa información

global right_password 
right_user = 51612 # Usuario predefinido
right_password = 21615 # Contraseña predefinida
user_easter_egg1 = "Tripulante2022" # Usuario usado para ejecutar el RF 1.1 del Easter Egg 1
password_easter_egg2 = "m1s10nt1c" # Contraseña usada para ejecutar el RF 1.1 del Easter Egg 2
...
def funciones_menu(right_password,coordenadas,bandera,informacion,opcubic): # Función que incluye todas las opciones del programa
    ...
    while contador_intentos < 3: # Si el usuario supera la cantidad de intentos fallidos (3), el programa termina su ejecución
        ...
        if (opcion_menu == 2021): # Si el usuario digita esta opción (2021), se ejecuta el RF 1.2 del Easter Egg 1
            lat = float(input(f"Dame una latitud y te diré de cuál hemisferio es... \n"))
            if (lat>0):
                print("Usted está en el hemisferio norte")
                time.sleep(2)
                funciones_menu(right_password,coordenadas,bandera,informacion,opcubic)
            else:
                print("Usted está en el hemisferio sur")
                time.sleep(2)
                funciones_menu(right_password,coordenadas,bandera,informacion,opcubic)

        if (opcion_menu == 2022): # Si el usuario digita esta opción (2022), se ejecuta el RF 1.2 del Easter Egg 2
            coord = float(input(f"Escribe una coordenada de una longitud en Sudamérica y te diré su huso horario\n"))
            buscar_huso_horario(coord)
...
def promedio_latitudes(cantlatitudes): # Funciona para calcular el promedio de las latitudes ingresadas al ejecutar el RF 1.1 del Easter Egg 2
    contador = 0
    for i in range(cantlatitudes):
        lat = float(input(f"Ingrese latitud {i+1}: "))
        contador = contador + lat
    promedio = contador/cantlatitudes
    return promedio

def buscar_huso_horario(coord): # Funciona para definir un huso horario de acuerdo a la coordenada ingresada por el usuario al ejecutar el RF 1.2 del Easter Egg 2
    if ((coord >= -81.296) and (coord <= -67.401)):
        print("El huso horario es -5")
    elif ((coord >= -67.402) and (coord <= -54.316)):
...
def programa(right_password,coordenadas,bandera,informacion,opcubic,user_easter_egg1,password_easter_egg2): # Programa completo, incluye todas las funciones

    print("Bienvenido al sistema de ubicación para zonas públicas WIFI")
    user = input("Ingrese el usuario: ")
    if user == user_easter_egg1: # Si el usuario ingresado es "Tripulante2022", se ejecuta el RF 1.1 del Easter Egg 1
        print("Este fue mi primer programa y vamos por más")
        exit()
    else:
        user = int(user) # En caso de que el usuario ingresado no sea un string como "Tripulante2022", el programa pasa a validar que sea un integer (51612 es el correcto)
        if right_user != user:
            print("Error")
            exit()
        else:
            password = input("Ingrese la contraseña: ")
            if password == password_easter_egg2: # Si la contraseña ingresada es "m1s10nt1c", se ejecuta el RF 1.1 del Easter Egg 2
                cantlatitudes = int(input(f"¿Cuántas latitudes deseas ingresar?\n"))
                promlatitudes = promedio_latitudes(cantlatitudes) # Calcula el promedio de latitudes ingresadas en la función
                print(f"El promedio es: {promlatitudes}")
                exit()
            else:
...
programa(right_password,coordenadas,bandera,informacion,opcubic,user_easter_egg1,password_easter_egg2) # Se ejecuta el programa completo